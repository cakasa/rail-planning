...
for SETTING in 3 4 5
do
    mkdir results/s$SETTING
    for PROBLEM in problem7b problem7c problem8b problem8c problem9b problem9c
    do
        ...
        timeout 30m ./ff-v2.1.exe -p ./ -o tusd.pddl -f problems/$PROBLEM.pddl -w 10 -s $SETTING >> results/s$SETTING/result-$PROBLEM-setting-$SETTING.out
