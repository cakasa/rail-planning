...
(:predicates 
    ...
    (entrance ?x - trackpart) ; track part x is the entrance/exit to the shunting yard
)
(:functions
...
(:action move-to-track
    ...
    :precondition (and (at ?train ?from) (free ?to) 
                    ...
                    (switch ?from) (>= (capacity ?t) (length ?train)))
...
(:action move-to-departure
    ...
    :precondition (and (at ?train ?from) (nextTo ?from ?to)
                    (entrance ?to) (hasBeenParked ?train))
    :effect (and (at ?train ?to) (not (at ?train ?from)) 
                    (free ?from) 
...
(:action move-on-arrival
    ...
    :precondition (and (at ?train ?from) (free ?to) 
                    ...
                    (entrance ?from) (>= (timestep) (arrive ?train)))
    :effect (and (at ?train ?to) (not (at ?train ?from)) 
                    (not (free ?to)) (increase (timestep) 1))
)
