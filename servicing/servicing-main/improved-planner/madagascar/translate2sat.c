...
fma *effectsofL(int *lits)
{
  ...
  {
    if ((*lits) & 1)
      fs = fmacons(Fneg(fmaVAR(NEXT(feVAR(*lits)))), fs);
    else
      fs = fmacons(fmaVAR(NEXT(feVAR(*lits))), fs);
    lits = lits + 1;
  }
...
void storeactvars(int var, int *effectlits, fma *preconlits)
{
  ...
  *ptr = -1;
  ...
  printUlits(actvars[var].conditionlits);
  printf("\n effects:");
  printUlits(actvars[var].effectlits);
  printf("\n");
#endif
}
...
void translateaction(int i)
{
  ...
  { /* Do the translation with variables for conditional effects. */

    while (e != NULL)
    {
      ef = effectsofL(e->effectlits);
      if (e->condition->t == TRUE)
...
void printvar(int v)
{
  ...
  {
  case AUXtag:
    printf("AUX%i", VARINDEX(v));
    break;
  case VARtag:
    printatomi(VARINDEX(v));
    break;
  case OPtag:
    printf("OP%i", VARINDEX(v));
    break;
  default:
    printf("(INCORRECT VAR TYPE %i", v);
    break;
  }
}
...
void SEQUmutexes()
{
  ...
  for (i = 0; i < nOfActions; i++)
  {
    ...
    {
      if (i & bit)
        addformula(transition, Fimpl(fmaOP(i), fmaAUX(firstindexbit + j)));
      else
        addformula(transition, Fimpl(fmaOP(i), Fneg(fmaAUX(firstindexbit + j))));
      bit = bit << 1;
    }
  }
}
...
void ESTEPchain(int i, sccs s, int WHAT)
{
  ...
  case 1:
    for (j = 0; j < nOfActions; j++)
    {
      if (canmaketrue(j, i))
        auxM[NM++] = j;
      if (isaffectedby(j, i) && Lparallel(i, j))
        auxR[NR++] = j;
    }
    break;
  case 2:
  ...
  case 0:
    ...
    {
      if (canmaketrue(s->els[j], i))
        auxM[NM++] = s->els[j];
      if (isaffectedby(s->els[j], i) && Lparallel(i, s->els[j]))
        auxR[NR++] = s->els[j];
    }
  ...
  if (NM * NR < 5000 && noparallels(NM, NR))
    return;
  ...
  {
    printf("%i ", auxM[j]);
    printactionname(auxM[j]);
  }
  printf("\n");
  for (j = 0; j < NR; j++)
  {
    printf("%i ", auxR[j]);
    printactionname(auxR[j]);
  }
  printf("\n");
  ...
  while (pM < NM || pR < NR)
  {
    ...
    if (pR < NR && op == auxR[pR])
    { /* Operator may need to be disabled. */
  ...
  { /* Go through all actions. */
    ...
    while (es != NULL)
    {
      ...
      while (*ptr != -1)
      {
        ...
        if ((*ptr) & 1)
          ptr2 = ApreconoccP[feVAR(*ptr)];
        else
          ptr2 = ApreconoccN[feVAR(*ptr)];
...
void emittemporarygoalinitclauses()
{
  ...
  while (cnt < tmpnOfClauses)
  {
    ...
    case TRtag:
      ...
      break;
    default:
      fprintf(stderr, "emittemporarygoalinitclauses");
      exit(1);
    }
...
int pprenamelit(int lit)
{ // Rename literal after using external preprocessor
  int negated;
  int index, newindex;
  int NAUXBEGIN;
  ...
  if (index < ppSVars + ppActions + ppAux)
  { // SVar, Action, Aux indexing unchanged
    newindex = index;
  }
  else if (index >= NAUXBEGIN)
  { // New auxiliaries will go after old auxiliaries.
    newindex = index - ppSVars;
  }
  else
  { // Next state variables will be shifted after new auxiliaries.
...
void docp3preprocessing()
{
  ...
  for (i = 0; i < nOfClauses; i++)
    if ((clauseset[ptr] & TIMEtags) == INITtag)
      testClss++;
  ...
  ptr = 0;
  for (i = 0; i < nOfClauses; i++)
    if ((clauseset[ptr] & TIMEtags) == TRtag)
      testClss++;
  ...
  ptr = 0;
  for (i = 0; i < nOfClauses; i++)
    if ((clauseset[ptr] & TIMEtags) == GOALtag)
      testClss++;
  ...
  for (freezeVar = 0; freezeVar < nOfAtoms; ++freezeVar)
  { // this time steps atom variables
    CPfreezeVariable(preprocessor, freezeVar);
  }
  printf("5\n");
  for (freezeVar = varsPerTime; freezeVar < varsPerTime + nOfAtoms; ++freezeVar)
  { // next time steps atom variables
    CPfreezeVariable(preprocessor, freezeVar);
  }
  if (1)
  { // should have a parameter here!
    printf("6\n");
    for (freezeVar = nOfAtoms; freezeVar < nOfAtoms + nOfActions; ++freezeVar)
    { // action variables -- not necessary to be frozen, however, disappear otherwise, can be reconstructed
      CPfreezeVariable(preprocessor, freezeVar);
    }
  }
  printf("7\n");
  ...
  { // printing all clauses, and checking whether the numbers actually work and all literals are printed
...
void encoding()
{
  ...
  {
    if (initialstate[i] == 1)
      addformula(inittime, fmaVAR(i));
    else
      addformula(inittime, Fneg(fmaVAR(i)));
  }

  addformula(goaltime, makeVARfma(goal));

  if (planSemantics == EStepOgata)
  { /* Do Ogata style encoding. */

    encodingOgata();
  }
  else
  { /* Do all other encodings. */
    ...
    // Add move-on-arrival and move-to-departure operators to fma
    // for (i = 0; i < nOfActions; i++)
    // {
    //   int *name = actions[i].name;

    //   if (strcmp(symbol(*name), "move-on-arrival") == 0)
    //   {
    //     int from = atomtable[actions[i].precon->juncts->tl->tl->tl->hd->a]->tl->tl->hd;
    //     int to = atomtable[actions[i].precon->juncts->tl->tl->hd->a]->tl->hd;
    //     if (strcmp(symbol(from), symbol(to)) > 0)
    //     {
    //       addformula(transition, fmaOP(i));
...
#define RANDOMORDER
  ...
  for (i = 0; i < nOfClauses; i++)
  {
    len = (clauseset[ptr]) & LENBITS;
    for (j = 1; j <= len; j++)
      clauseset[ptr + j] = final(clauseset[ptr + j], 0);
    ptr = ptr + len + 1;
  }
  ...
  else
  {
    ...
    if (PLANheuristic == 0)
      printf("Heuristic: VSIDS\n");

    nofshortcutclauses = 0;
    if (flagShortCutHorizon)
      addshortcuts();

    switch (evalAlgorithm)
    {
    case 0:
      runalgorithmA(paramA, outputTimeStep);
      break;
    case 1:
      runalgorithmB(paramB, outputTimeStep);
      break;
    case 2:
      runalgorithmC();
      break;
...
void startlength(int i, int len)
{
  ...
  if (i > 0)
    seqs[i].sati->al2its = seqs[0].sati->al2its;
}
...
#pragma omp critical
#endif
  {
    ...
    if (seqs[i].sati->value == 0)
    {
      ...
      for (j = 0; j < i; j++)
      {
        if (seqs[j].sati->value == -1)
        { /* Formulas that must be UNSAT. */
          seqs[j].sati->value = 0;
...
void runalgorithmA(int n, int step)
{
  ...
  do
  {
   ...
   we would otherwise need to redirect pointers in the decision stack. */
      for (j = 0; j <= last; j++)
        if (seqs[j].sati->value == -1 && seqs[j].callsleft && computeonestep(j, 1))
          return;
...
#pragma omp atomic
       ...
       we would otherwise need to redirect pointers in the decision stack. */
        ...
        if ((instancelength(last + 1) > lastTimePoint) && (seqs[last].sati->value != -1))
          NOPLAN = 1;
...
void runalgorithmB(double r, int step)
{
  ...
  do
  {
    ...
    if (last > -1)
      threshold = ((float)(seqs[firstactive].effort)) * power(r, last + 1 - firstactive);
...
inline int final(int i, int time)
{
  ...
  case OPtag:
    ...
    break;
  default:
    fprintf(stderr, "ERROR: 43346 %i %i\n", i % TYPEtag, i);
    exit(1);
    break;
  }
  if (i & NEXTtag)
...
inline int finalNOTIME(int i)
{
  ...
  case OPtag:
    ...
    break;
  default:
    assert(1 == 0); // fprintf(stderr,"ERROR: 43346\n"); exit(1); break;
  }
  return j + VARINDEX(i);
}
...
void emitclause(int *vector, int cnt, formulaclass class)
{
  ...
  if (class == transition)
    nOfTClauses += 1;
  if (clauseptr + cnt + 1 > maxClauseEls)
    reallocclauseset(clauseptr + cnt + 1);
  realcnt = 0;
...
int biggerthan0(fma *f, int bound, int *size)
{
  ...
  case conj:
    ...
    *size = 0;
    while (fs != NULL)
    {
      if (biggerthan0(fs->hd, bound, &size0))
        return 1;
      *size += size0;
      if (*size > bound)
        return 1;
  ...
  while (i <= l0)
  {
    ...
    while (j <= l1)
    {
      ...
      *l2 = clausealloc; /* Index of the last clause generated */
      ...
      for (k = 1; k <= clauselist[i]; k++)
        clauselist[clausealloc++] = clauselist[i + k];
      for (k = 1; k <= clauselist[j]; k++)
        clauselist[clausealloc++] = clauselist[j + k];
      j += clauselist[j] + 1;
    }
    i += clauselist[i] + 1;
  }
...
void pc30(fma *f, formulaclass cl, int *first, int *last)
{
  ...
  case disj:
    ...
    else
    {
      ...
      fs = fs->tl;
      while (fs != NULL)
      { /* Repeatedly construct the product. */
        pc30(fs->hd, cl, &f1, &l1);
  ...
  case conj:
    fs = f->juncts;
    if (fs == NULL)
    { /* Empty conjunction is the empty clause set. */
      *first = 0;
      *last = -1;
      return;
    }
...
void outputDIMACS()
{
  ...
  for (i = firstTimePoint; i <= lastTimePoint; i += outputTimeStep)
  { /* Write files for different lengths. */
    if (filenamebase)
    {
    ...
    for (j = 0; j <= i; j++)
    { /* Go through time points for one formula. */
      printf(" %i", j);
      fflush(stdout);
      ptr = 0;
      bias = j * varsPerTime;
      for (k = 0; k < nOfClauses; k++)
      { /* Output clauses for each time point. */
        with = 0;
        len = clauseset[ptr] & LENBITS;
        switch (clauseset[ptr] & TIMEtags)
        { /* 1st element = size, timepoints */
        case TRtag:
          if (j < i)
            with = 1;
          break;
      ...
      if (j > 0)
      {
        for (k = 0; k < nOfAtoms; k++)
        {
          ...
          {
            if (feVAR(h) < k)
            { /* Only output one of l1 V l2 and l2 V l1. */
              if (h & 1)
...
void printClausesWithType(int type, FILE *F)
{
  ...
  { /* Output clauses for each time point. */
    ...
    if ((clauseset[ptr - 1] & TIMEtags) == type)
    { /* 1st element = size, timepoints */
      for (h = ptr; h < ptr + len; h++)
      {
        if (clauseset[h] < 0)
          fprintf(F, "%i ", clauseset[h]);
        else
          fprintf(F, "%i ", clauseset[h]);
      }
      fprintf(F, "0\n");
...
satinstance outputNSAT(int id, int timepoints, int TRonly)
{
  ...
  { /* Output clauses. */
    ...
    if ((TRonly == 0) || (ct == TransC))
    {
      ...
      {
      case 1:
        add1clause(sati, nsatlit(clauseset[ptr]), ct);
        break;
      case 2:
  ...
  if (noT2clauses)
    sati->al2its = seqs[id - 1].sati->al2its;
