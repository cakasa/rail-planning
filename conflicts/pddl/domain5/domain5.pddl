...
(:predicates 
    ...
    (canPark ?x - trackpart)
    (trainParkingTrack ?train - trainunit ?track - track)
    (departing)
    (driverCanSwitch)
    (station ?x - trackpart)
)
...
(:action move-on-arrival
    ...
    :precondition (and (driver ?train) (not (departing)) (at ?train ?from) 
                  (free ?to) (nextToArrival ?from ?to) (forall (?track - track) (not (onTrack ?to ?track))))
...
(:action park-train
    ...
    :effect (and (hasBeenParked ?train) (parkedOn ?train ?t) (driverCanSwitch) (not (canPark ?loc)) (canPark ?newparkingspot))
)

(:action driver-switch-train
    :parameters (?from ?to - trainunit)
    :precondition (and (driverCanSwitch) (hasBeenParked ?from) (driver ?from) (or 
        (and (nextTrainArriving ?from ?to) (not (departing))) 
        (and (nextTrainDeparting ?from ?to) (departing))))
    :effect (and (not (driver ?from)) (driver ?to) (not (driverCanSwitch)))
)

(:action driver-switch-train-start-departing
    :parameters (?from ?to - trainunit)
    :precondition (and (driverCanSwitch) (not (departing)) (driver ?from) (lastArrival ?from) (hasBeenParked ?from) (firstDeparture ?to))
...
(:action depart-train
    ...
    :effect (and (free ?loc) (hasDeparted ?train) (driverCanSwitch))
)
