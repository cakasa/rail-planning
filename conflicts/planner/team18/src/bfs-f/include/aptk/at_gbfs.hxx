...
public:
	...
	Abstract_Heuristic*		get_heuristic() 		{ return m_heuristic_func; }

	void			eval( Search_Node* candidate ) {
		// const void &statereference = ;
		unsigned eval;
		m_heuristic_func->eval(*(candidate->state()), eval );
		float& candidate_hn = candidate->hn();
		candidate_hn = (float)eval;
	}
