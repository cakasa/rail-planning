...
public:
	...
	void			eval( Search_Node* candidate ) {
		...
		if ( candidate->goals_unachieved() == 0){
			...
			// 	const aptk::Action& act = *(m_problem.task().actions()[candidate->action()]);
			// 	candidate->set_state(candidate->parent()->state()->progress_through(act));
			// }

			// m_first_h->eval( *(candidate->state()), candidate->h1n() );
