...
namespace aptk
{
	...
	public:
		...
		const std::string fluent_pred_names(unsigned f) const {
			return m_fluent_pred_name.at(f);
		}
		void set_fluent_pred_names(unsigned f, std::string str) {
			m_fluent_pred_name[f] = str;
		}
		const std::vector<std::string> fluent_pred_args(unsigned f) const {
			return m_fluent_pred_args.at(f);
		}
		void set_fluent_pred_args(unsigned f, std::vector<std::string> str) {
			m_fluent_pred_args[f] = str;
		}
	...
	protected:
		...
		std::map<int, std::string>              m_fluent_pred_name;
		std::map<int, std::vector<std::string>> m_fluent_pred_args;
