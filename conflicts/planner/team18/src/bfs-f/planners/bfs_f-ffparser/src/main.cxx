...
using	aptk::agnostic::Train_Heuristic;
...
using	aptk::search::Node_Comparer_Greedy;
...
typedef 	aptk::search::bfs::Node< aptk::State > Search_Node_1h;
...
typedef 	Node_Comparer_Greedy< Search_Node_1h >					Tie_Breaking_Algorithm_1h;			
...
typedef		Open_List< Tie_Breaking_Algorithm_1h, Search_Node_1h >		BFS_Open_List_1h;

// MRJ: Now we define the heuristics
typedef 	Train_Heuristic<Fwd_Search_Problem> H_train;
...
typedef		AT_GBFS_3H< Fwd_Search_Problem, H_train, H_Lmcount_Fwd, Classic_FF_H_Max, BFS_Open_List >    	Anytime_Train;
typedef     AT_GBFS_SQ_SH< Fwd_Search_Problem, H_train, BFS_Open_List_1h > 									Anytime_Train_1h;
...
int main( int argc, char** argv ) {
	...
	std::cout << "\t--------------------------------- Im building" << std::endl;

	std::cout << "Printing parsed problem to /mnt/STRIPS_domain.txt" << std::endl;
	std::ofstream domain_file;
	domain_file.open("/mnt/STRIPS_domain.txt");
	domain_file << "Actions: " << std::endl;
	prob.print_actions(domain_file);
	domain_file << "Fluents: " << std::endl;
	prob.print_fluents(domain_file);
	domain_file << "Fluents vec: " << std::endl;
	prob.print_fluent_vec(domain_file, prob.init());
	domain_file.close();

	std::map<std::string, int> train_depart;
	std::map<std::string, int> trackpart_depth;

	std::ifstream domain_stream;
	domain_stream.open(vm["problem"].as<std::string>());
	std::string line;	
	while (getline(domain_stream, line))
	{
		if (line.find("trainunit") != std::string::npos && train_depart.size() == 0) {
    		...
    		for (auto token : tokens) {
				if (token == "-") {
					...
					train_depart[boost::to_upper_copy(token)] = -1;
				}
			}
		}

		if (line.find("trackpart") != std::string::npos) {
			std::vector<std::string> tokens;
			boost::split(tokens, line, boost::is_any_of("\t "));
			tokens.erase(remove(tokens.begin(), tokens.end(), ""), tokens.end());
    		for (auto token : tokens) {
				if (token == "-") {
					...
					trackpart_depth[boost::to_upper_copy(token)] = -1;
				}
			}
		}

		if (line.find("depart") != std::string::npos)  {
			std::vector<std::string> tokens;
			boost::split(tokens, line, boost::is_any_of("\t ()"));
			tokens.erase(remove(tokens.begin(), tokens.end(), ""), tokens.end());
			
			train_depart[boost::to_upper_copy(tokens[2])] = stoi(tokens[3]);
		}
		...
		if (line.find("depth") != std::string::npos) {
			...
			boost::split(tokens, line, boost::is_any_of("\t ()"));
			tokens.erase(remove(tokens.begin(), tokens.end(), ""), tokens.end());
			trackpart_depth[boost::to_upper_copy(tokens[2])] = stoi((tokens[3]));
		}
	}
    domain_stream.close();

	std::cout << "Track parts depths: " << std::endl;
	for(const auto& elem : trackpart_depth)
	{
		std::cout << elem.first << " " << elem.second << " " << std::endl;
	}

	std::cout << "Track train departs: " << std::endl;
	for(const auto& elem : train_depart)
	{
		std::cout << elem.first << " " << elem.second << " " << std::endl;
	}
	...
	if(vm["use-original-ff-heur"].as<bool>()){
		...
		// std::cout << "Starting search with BFS(train,land,h_add)..." << std::endl;

		// Anytime_Train bfs_engine( search_prob );
		// bfs_engine.h3().set_one_HA_per_fluent( vm["one-ha-per-fluent"].as<bool>() );
		...
		// float bfs_t = do_search( bfs_engine, prob, 0, plan_stream );
		
		// std::cout << "BFS search completed in " << bfs_t << " secs" << std::endl;

		std::cout << "Starting search with BFS(train)..." << std::endl;
		Anytime_Train_1h bfs_engine( search_prob );
		bfs_engine.get_heuristic()->set_problem_info(train_depart, trackpart_depth);
		float bfs_t = do_search( bfs_engine, prob, 0, plan_stream );
		std::cout << "BFS search completed in " << bfs_t << " secs" << std::endl;
	}
