def create_problem_file(filename, problem_name, trains, arrival_order, departure_order, tracks, next_to):
    ...
    with open(filename, 'w+') as f:
        f.write("(define (problem {}) (:domain servicing-no-fluents)\n".format(problem_name))
        
        ##### define objects #####
        f.write("(:objects\n")

        # initialize the train units
        for train_name, (train_type, _, _) in trains.items():
            f.write("    " + train_name + " - " + train_type +"\n")
        
        # calculate the total length of the trains
        total_train_length = 0
        for _, (_, train_length, _) in trains.items():
            total_train_length += train_length

        # initialize the track parts of the arrival/departure path
        track_parts_string = "    "
        for i in range(total_train_length):
        ...
        for train in arrival_order:
            _, train_length, needed_services = trains[train]
            for i in range(train_length):
                f.write("    (at {} v{})\n".format(train, init_id + i))
            f.write("    (beginsAt {} v{})\n".format(train, init_id))
            f.write("    (endsAt {} v{})\n".format(train, init_id + train_length - 1))
            init_id += train_length
        ...
        for i in range(1, total_train_length):
            f.write("    (nextTo v{} v{})\n".format(i, i + 1))
            f.write("    (nextTo v{} v{})\n".format(i + 1, i))
